-- PostgreSQL Schema for IMDb Datasets
...
CREATE TABLE IF NOT EXISTS name_basics (
    ...
    birth_year INTEGER,  -- in YYYY format, NULL if not available
    death_year INTEGER,  -- in YYYY format if applicable, NULL if not applicable
    primary_profession TEXT[],  -- array of strings, the top-3 professions of the person
    known_for_titles TEXT[]  -- array of tconsts, titles the person is known for
);
...
CREATE TABLE IF NOT EXISTS title_akas (
    ...
    types TEXT[],  -- Enumerated set of attributes for this alternative title
...
CREATE TABLE IF NOT EXISTS title_ratings (
    ...
    num_votes INTEGER NOT NULL  -- number of votes the title has received
);
...
COMMENT ON TABLE name_basics IS 'Basic information about names/persons in IMDb';
COMMENT ON TABLE title_basics IS 'Basic information about titles (movies, TV shows, etc.) in IMDb';
COMMENT ON TABLE title_akas IS 'Alternative titles for movies/shows in different languages/regions';
COMMENT ON TABLE title_crew IS 'Director and writer information for titles';
COMMENT ON TABLE title_episode IS 'Episode information for TV series';
COMMENT ON TABLE title_principals IS 'Principal cast/crew for titles';
COMMENT ON TABLE title_ratings IS 'Rating information for titles';
...
LEFT JOIN title_ratings tr ON tb.tconst = tr.tconst
WHERE 'nm0000233' = ANY(tc.directors)  -- Christopher Nolan
